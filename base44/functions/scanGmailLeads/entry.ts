...
const SHEET_HEADERS = ['שם מלא', 'טלפון', 'מייל', 'מקור', 'שירות / עניין', 'סטטוס', 'התקדמות', 'הערות'];
...
function websiteSheetStatus(lead) {
...
function buildSheetRow(lead) {
  ...
  return [
    ...
    lead.email || '',
    lead.source || 'natigold.com (אתר)',
    lead.shooting_type || lead.lead_type || '',
    websiteSheetStatus(lead),
    lead.pipeline_stage === 'lead_found' ? 'ליד נמצא' : (lead.pipeline_stage || ''),
    notes,
  ];
}
...
function rowMatchesLead(row, lead) {
  ...
  const leadUrl = normalize(lead.source_post_url);
  ...
  if (leadName && normalize((row || [])[0]) === leadName) return true;
  return false;
}

async function ensureSheetHeaders(authHeader, tabName) {
  const range = encodeURIComponent(`'${tabName}'!A1:H1`);
  const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${range}`, { headers: authHeader });
...
async function deleteExistingSheetRows(authHeader, lead) {
  const tabs = [ALL_LEADS_TAB, SHEET_TAB];
  ...
  const ranges = tabs.map((tab) => `ranges=${encodeURIComponent(`'${tab}'!A1:Z3000`)}`).join('&');
...
async function appendSheetRow(authHeader, tabName, row) {
  await ensureSheetHeaders(authHeader, tabName);
  const status = row[5] || '';
  ...
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(`'${tabName}'!A:H`)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`;
...
async function syncWebsiteLeadToSheets(base44, lead) {
  const { accessToken } = await base44.asServiceRole.connectors.getConnection('googlesheets');
  const authHeader = { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' };
  await deleteExistingSheetRows(authHeader, lead);
  const row = buildSheetRow(lead);
  await appendSheetRow(authHeader, ALL_LEADS_TAB, row);
  await appendSheetRow(authHeader, SHEET_TAB, row);
}
  ...
  try {
    ...
    for (const l of parsed) {
      ...
      if (existing) {
        ...
        const sheetLead = Object.keys(updateData).length > 0
          ? { ...existing, ...updateData }
          : existing;
        ...
        leadsForSheetSync.push(sheetLead);
        continue;
      }
    ...
    if (newLeads.length > 0) {
      ...
      const createdRecords = Array.isArray(created) ? created : rows;
      createdLeads.push(...createdRecords);
      leadsForSheetSync.push(...createdRecords);
    }

    let sheetSynced = 0;
    for (const leadToSync of leadsForSheetSync) {
      try {
        await syncWebsiteLeadToSheets(base44, leadToSync);
        sheetSynced++;
      } catch (syncError) {
        console.error(`scanGmailLeads: sheet sync failed for "${leadToSync.name}": ${syncError.message}`);
      }
    }
    ...
    await base44.asServiceRole.entities.SystemLog.create({
      action: 'gmail_lead_scan',
      details: JSON.stringify({
        summary: `נסרקו ${idsToProcess.length} מיילים — ${newLeads.length} לידים חדשים, ${updatedCount} עודכנו, ${sheetSynced} סונכרנו לגוגל שיטס`,
        found: idsToProcess.length,
        saved: newLeads.length,
        updated: updatedCount,
        sheet_synced: sheetSynced,
        sheet_candidates: leadsForSheetSync.length,
        leads: leadDetails,
      }),
      status: 'success',
    });
