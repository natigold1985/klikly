...
Deno.serve(async (req) => {
  try {
    ...
    for (const link of links) {
      if (!link.wants_reminders || link.fully_saved_at) continue;
      const consentAt = new Date(link.reminder_consent_at || link.updated_date || link.created_date);
      if (Number.isNaN(consentAt.getTime()) || now - consentAt < HOURS_48) continue;

      const lastSent = link.last_notification_sent ? new Date(link.last_notification_sent) : null;
      if (lastSent && now - lastSent < HOURS_48) continue;

      const clientName = link.client_name || 'לקוח יקר';
      const projectTitle = link.project_title || 'הגלריה שלך';
      const galleryPath = `/g/${link.token}`;
      const message = `היי ${clientName}, תזכורת ידידותית לוודא ששמרת את כל הקבצים מהגלריה: ${projectTitle}. ${galleryPath}`;
      ...
      await base44.asServiceRole.entities.SystemLog.create({
        action: 'client_gallery_reminder_sent',
        details: `Reminder sent for ${clientName}. WhatsApp link: ${waLink || 'no phone'}`,
        status: 'success',
