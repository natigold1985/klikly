...
Deno.serve(async (req) => {
  try {
    ...
    // Source of truth: TeamMember
    const allMembers = await base44.asServiceRole.entities.TeamMember.list('-created_date', 1000);
    let clients;
    if (isAdmin) {
      clients = allMembers.filter(u => u.role === 'client');
    } else {
      clients = allMembers.filter(u => u.role === 'client' && u.assigned_photographer_email === me.email);
    }

    // File counts per client
