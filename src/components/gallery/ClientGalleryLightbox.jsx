...
import { ChevronLeft, ChevronRight, Heart, Loader2, MessageSquare, X } from 'lucide-react';
...
export default function ClientGalleryLightbox({ photos, index, setIndex, onClose, selectedIds, toggleSelection, savingPhotoId, photoComments, setPhotoComments }) {
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/95 backdrop-blur-xl flex flex-col" dir="rtl">
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 grid-rows-[minmax(0,1fr)_auto] lg:grid-cols-[1fr_340px] lg:grid-rows-1 gap-0 pt-16 pb-24 lg:pb-4">
          ...
          <aside className="lg:my-4 lg:ml-4 mx-3 lg:mx-0 bg-[#0a0a0a] border border-white/10 rounded-2xl p-4 md:p-5 shadow-2xl flex flex-col gap-4 max-h-[42vh] lg:max-h-none overflow-y-auto">
            ...
            <button disabled={savingPhotoId === photo.id} onClick={() => toggleSelection(photo.id)} className={`w-full flex items-center justify-center gap-2 px-5 py-4 rounded-xl transition-all active:scale-95 border disabled:opacity-70 ${isSelected ? 'bg-[#FFD700] text-black border-[#FFD700]' : 'bg-white/5 text-white hover:bg-white/10 border-white/10'}`}>
              {savingPhotoId === photo.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Heart className={`w-5 h-5 ${isSelected ? 'fill-black' : ''}`} />}
              <span className="font-bold text-base md:text-lg">{savingPhotoId === photo.id ? 'שומר...' : isSelected ? 'נבחרה' : 'סמן כמועדפת'}</span>
            </button>
            {isSelected && (
